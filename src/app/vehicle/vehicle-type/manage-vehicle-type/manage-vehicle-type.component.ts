...
export class ManageVehicleTypeComponent implements OnInit {
  ...
  this.shared.toEdit = row.id;
    this.router.navigate([`/vehicle/edit-vehicle-type`]);
  }

  deleteCall(row: any) {
    let name = row.vehicleTypeName
  ...
  deleteRow(id) {
    this.vehicleService.deleteVehicleType(id).subscribe((res) => {
  ...
  exelExport() {
    this.exporter.exportTable(ExportType.XLSX, {
      fileName: "KPR_Vehicle_Type_Details_" + this.currentDateTime,
    });
  }
  ...
  pdfExport() {
    ...
    doc.text("KPR Vehicle Type Details", 15, 25);
    ...
    autoTable(doc,
      {
        ...
        body: data,
        columns: [
          { header: "Vehicle Type Name", dataKey: "vehicleTypeName" },
          { header: "Description", dataKey: "description" },

        ],
      }
    );
    doc.save("KPR_Vehicle_Type_Details_" + this.currentDateTime);
  }
