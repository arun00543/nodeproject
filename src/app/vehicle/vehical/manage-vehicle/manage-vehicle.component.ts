...
export class ManageVehicleComponent implements OnInit {
  ...
  this.shared.toEdit = row.id;
    this.router.navigate([`/vehicle/edit-vehicle`]);
  }

  deleteCall(row: any) {
    let name = row.vehicleType.vehicleTypeName
  ...
  exelExport() {
    this.exporter.exportTable(ExportType.XLSX, {
      fileName: "KPR_Vehicle_Details_" + this.currentDateTime,
    });
  }
  ...
  pdfExport() {
    ...
    doc.text("KPR Vehicle Details", 15, 25);
    ...
    autoTable(doc,
      {
        ...
        columns: [
          ...
          { header: "Vehicle Number", dataKey: "vehicleRegistrationNumber" },
          { header: "Vehicle Make", dataKey: "vehicleMake" },
        ...
        didParseCell: function (data) {
          if (data.column.dataKey === 'vehicleType') {
            var text = data.row.raw["vehicleType"].vehicleTypeName;
            if (text && text.length > 0) {
              data.cell.text = text;
            }
            var text1 = data.row.raw['vehicleType'].vehicleRegistrationNumber;
            if (text1 && text1.length > 0) {
              data.cell.text = text1;
            }
          }
          if (data.column.dataKey === 'purchasePrice') {
            var text;
            if("Purchase Price" != data.row.raw["purchasePrice"]) {
             text = new IndCurrencyFormat().transform(data.row.raw["purchasePrice"]).replace('₹','Rs.');
            } else {
              text = data.row.raw["purchasePrice"]
            }
            if (text && text.length > 0) {
              data.cell.text = text;
            }            
    ...
    doc.save("KPR_Vehicle_Details_" + this.currentDateTime);
  }
  sortData(event: Sort) {
