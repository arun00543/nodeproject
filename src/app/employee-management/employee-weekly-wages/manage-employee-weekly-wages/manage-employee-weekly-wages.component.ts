...
export class ManageEmployeeWeeklyWagesComponent implements OnInit {
    ...
    autoTable(doc,
      {
        ...
        didParseCell: function (data) {
          ...
          if (data.column.dataKey === 'weeklyTotalPay') {
            var text;
            if("Weekly Total Pay" != data.row.raw["weeklyTotalPay"]) {
             text = new IndCurrencyFormat().transform(data.row.raw["weeklyTotalPay"]).replace('₹','Rs.');
            } else {
              text = data.row.raw["weeklyTotalPay"]
            }
            if (text && text.length > 0) {
              data.cell.text = text;
            }            
  ...
  public loadData() {
   ...
   importExport() {
    ...
    this.router.navigate([`/administrative/weekly-wages/excel-import-export`]);
  }
