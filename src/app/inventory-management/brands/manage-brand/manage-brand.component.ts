...
export class ManageBrandComponent implements OnInit {
  ...
  this.shared.toEdit = row.id;
    this.router.navigate([`/inventory/edit-brand`]);
  }
  ...
  deleteRow(id) {
    ...
    this.inventoryService.deleteBrand(id).subscribe((res) => {
      this.loadData();
  ...
  exelExport() {
    this.exporter.exportTable(ExportType.XLSX, {
      fileName: "KPR_Brand_Details_" + this.currentDateTime,
    });
  }
  ...
  pdfExport() {
    ...
    doc.text("KPR Brand Details ", 15, 25);
    ...
    doc.save("KPR_Brand_Details_" + this.currentDateTime);
  }
