...
export class ManageCategoryComponent implements OnInit {
  ...
  this.shared.toEdit = row.id;
    this.router.navigate([`/inventory/edit-category`]);
  }
  ...
  deleteRow(id) {
    ...
    this.inventoryService.deleteitemCategory(id).subscribe((res) => {
      this.loadData();
  ...
  exelExport() {
    this.exporter.exportTable(ExportType.XLSX, {
      fileName: "KPR_Category_Details_" + this.currentDateTime,
    });
  }
  ...
  pdfExport() {
    ...
    doc.text("KPR Category Details ", 15, 25);
    ...
    autoTable(doc,
      {
        ...
        columns: [
          ...
          { header: "Category Description", dataKey: "categoryDescription" },
          

        ],
      }
    );
    doc.save("KPR_Category_Details_" + this.currentDateTime);
  }
